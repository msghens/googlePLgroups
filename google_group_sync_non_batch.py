...
FROM sfrstcr
WHERE
sfrstcr_term_code in (  Select      
             Stvterm_Code
             FROM   SORRTRM, SOBTERM, stvterm
             Where  Trunc (Sysdate) Between Sorrtrm_Start_Date And Sorrtrm_End_Date
             And Sorrtrm_Term_Code = stvterm_Code
             AND Sobterm_Dynamic_Sched_Term_Ind = 'Y'
             And Sobterm_Reg_Allowed = 'Y'
             And Sobterm_Term_Code = Stvterm_Code group by Stvterm_Code)
AND sfrstcr_rsts_code IN
  (SELECT stvrsts_code FROM stvrsts WHERE stvrsts_voice_type = 'R'
  )
"""
...
#                       os.makedirs(credential_dir)
	...
	gmuser_service = directory_service
	return gmuser_service


@logger.catch
def createGoogGroup(service,ggroup):
	groupKey = ggroup + '@pipeline.sbcc.edu'
	params = { "email" : groupKey,
				"name": groupKey}
		...
		while True:
				try:
						...
						if 'groupKey' in reason:
							...
							createGoogGroup(service,ggroup)
						#sys.exit()
						# raise
						break
				else:
...
def main(args):
	...
	for course in sbcc_classes:
		...
		try:
			...
			createGoogGroup(goog_service,course['BOM_COURSE_ID'])
			pass
		#~ print
		try:
			...
			if tosend:
				tomanny += 1    
				logger.info(f"Adding users to: {course['BOM_COURSE_ID']+'@pipeline.sbcc.edu'}")
				for i in tosend:
					try:
						logger.info(f"Adding: {i}")
						member_body = { "email": i}
						insert_member_google(goog_service,i,course['BOM_COURSE_ID']+'@pipeline.sbcc.edu')
					except Exception as e:
						reason = e._get_reason()
		...
		try:
			...
			if tosend:
				...
				for i in tosend:
					...
					delete_member_google(goog_service,i,course['BOM_COURSE_ID']+'@pipeline.sbcc.edu')
		except Exception as e:
